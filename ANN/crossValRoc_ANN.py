...
def K_fold_optimal_model2(data, labels, splits, optimal_dim):
    """
    Function that takes the optimal number of nodes from the cross_validation_ANN.py file and uses this to calcuate
    the area under the curve to determine how well the model perfoms
    :param data: the dataset
    :param labels: teh class labels
    :param splits: number of splits in teh data
    :param optimal_dim: the optimal ANN structure
    :return: the mean false positive rate, meean true positive rate and mean area under the curve.
    """
...
def draw_mean_optimal_ROC(means):
    """
    Function to draw an optimal ROC curve based on the mean data passed in from the classifer. 
    :param means:
    :return:
    """
    m1 = means[0]
